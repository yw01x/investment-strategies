...
- Appendix A: Q2(a) stock-level growth rates (`hw5_q2a_growth_rates.csv`)
- Appendix B: Q2(a) growth-rate summary (`hw5_q2a_growth_summary.csv`)
- Appendix C: Q2(b) equal-weight portfolio metrics (`hw5_q2b_equal_weight_metrics.csv`)
- Appendix D: Q2(c) equal-weight versus HW4 GMV comparison (`hw5_q2c_portfolio_comparison.csv`)
- Appendix E: compact summary metrics (`hw5_summary_metrics.csv`)
- Appendix F: selected code snippets from `code/hw5_analysis.py`
...
- The main numerical calculations and CSV exhibits for Q2(a), Q2(b), and Q2(c) are implemented in `code/hw5_analysis.py`.
- The two SVG figures used in the write-up are generated by `code/generate_hw5_figures.py`.
...
- Appendix F reproduces representative code snippets from `code/hw5_analysis.py` so that the computational logic is visible in the write-up itself, as in HW4.
...
### Step 1. Any optimizer of $(U_{\lambda})$ solves a variance-constrained return-maximization problem
...
### Step 2. For $\lambda > 0$, any optimizer of $(U_{\lambda})$ is efficient; $\lambda = 0$ gives the frontier endpoint

I first treat the case $\lambda > 0$ and show that $x_{\lambda}$ cannot be dominated by another feasible portfolio.
...
This proves efficiency for every optimizer of $(U_{\lambda})$ with $\lambda > 0$.
...
Call that selected portfolio $x_0^{\min \sigma}$. If there were some feasible $y \in \mathcal{S}$ with
...
- $\mu'y > \mu'x_0^{\min \sigma}$, contradicting the fact that $x_0^{\min \sigma}$ solves $(U_0)$; or
- $\mu'y = \mu'x_0^{\min \sigma}$ and $y'\Sigma y < (x_0^{\min \sigma})'\Sigma x_0^{\min \sigma}$, contradicting the minimum-variance tie-break among return-maximizing portfolios.

So $x_0^{\min \sigma}$ is the maximum-return endpoint of the efficient frontier.
...
- $\gamma \ge 0$ is the multiplier on the variance constraint,
...
where $\gamma^{\ast}$ is the optimal multiplier on the variance constraint.

There are two cases:

1. **The variance constraint binds.**  
   Then $\gamma^{\ast} > 0$ is the shadow value of relaxing the variance cap, and choosing $\lambda = \gamma^{\ast}$ makes the first-order conditions identical.

2. **The variance constraint does not bind.**  
   Then complementary slackness implies $\gamma^{\ast} = 0$, so the constrained problem reduces locally to pure expected-return maximization over $\mathcal{S}$. That is exactly $(U_0)$; if there are multiple return-maximizers, the efficient-frontier endpoint is obtained by choosing the minimum-variance one among them.
...
- Every optimizer of the risk-aversion problem $(U_{\lambda})$ with $\lambda > 0$ solves a reduced-form return-maximization problem with the variance cap set equal to its own realized variance and is efficient.
...
- Every optimizer of the reduced-form return-maximization problem $(P_{\sigma_{\max}})$ solves the risk-aversion problem $(U_{\lambda})$ for $\lambda$ equal to the KKT multiplier on the variance constraint.
...
The Q2(a) growth-rate construction and stability calculations are implemented in `code/hw5_analysis.py`; Appendix F.1 reproduces the representative snippet. The code excerpt below is taken from that file:
...
The compact metric summary reported in this subsection is provided as Appendix B (`hw5_q2a_growth_summary.csv`).

#### Representative stock examples

Table 1 gives a few representative reversals and improvements. The full stock-by-stock exhibit is provided as Appendix A (`hw5_q2a_growth_rates.csv`).
...
The daily rebalanced equal-weight portfolio and turnover calculations are implemented in `code/hw5_analysis.py`; Appendix F.2 reproduces the representative snippet. The code excerpt below is taken from that file:
...
The full CSV version of Table 2 is provided as Appendix C (`hw5_q2b_equal_weight_metrics.csv`).
...
The Q2(c) comparison is assembled in `code/hw5_analysis.py`; Appendix F.3 reproduces the representative snippet. The code excerpt below is taken from that file:
...
The full CSV version of Table 3 is provided as Appendix D (`hw5_q2c_portfolio_comparison.csv`).
...
Appendix A through Appendix D are supplied as separate CSV exhibit files in `exhibits/`. Appendix E and Appendix F are reproduced below inside the write-up so that the most grading-relevant summary numbers and representative implementation logic are visible in one document.
